...
<div class="sp-wrap">
    ...
    <div class="sp-header">
        ...
        @else
        <p class="sp-header-eyebrow">Collection</p>
        <h1 class="sp-header-title">Bộ Sưu Tập</h1>
        <div class="ix-rule" style="margin:16px auto 20px;"></div>
        <p class="sp-header-sub">
            Khám phá những kiệt tác hương thơm tinh tuyển,<br>
        ...
        <aside class="sp-sidebar">
            <form id="filterForm" action="{{ url()->current() }}" method="GET">
                ...
                <div class="sp-filter-header">
                    <span>Bộ lọc</span>
                    <a href="{{ url()->current() }}">Xóa tất cả</a>
                </div>
        ...
        <div class="sp-content">
            ...
            <div id="product-container">
                <div class="sp-grid">
                        ...
                        if (isset($festival)) {
                            ...
                            $maxDiscount = $product->festivals
                                ? $product->festivals->where('status', 1)
                                    ->filter(fn($f) => $f->start_date->toDateString() <= $today && $f->end_date->toDateString() >= $today)
                                    ->max('discount') ?? 0
                                : 0;
                        }
                    ...
                    <a href="{{ route('single_product', ['id' => $product->id]) }}" class="sp-card">
